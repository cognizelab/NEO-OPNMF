...
if ~isempty(parameter.coefficient) & ~isfield(Model,'group_level_RMSE_permutation_average') 
    ...
    for show = 1:length(parameter.coefficient)
        for g = 1:size(Model.group_level_W,1)
            ...
            figure('name',s);
            [newmatrix,showitem,ml] = plot_factor_loading(FL,SI,ID,items,rorder,flabel,itemcolor,dimlabel,itemsize);
...
%% coefficient
function [newmatrix,showitem,ml] = plot_factor_loading(FL,SI,ID,items,rorder,flabel,itemcolor,dimlabel,itemsize)
%% Input
